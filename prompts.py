...
You have access to the tool `send_email` (arguments `to_email`, `from_email`, `subject`, `body`, `attachment_files`) to send emails via SMTP.  
...
- attachment_files: List of attachment files. This **MUST** include the CSV file generated by the Writer Agent, without path (e.g., ["report.csv"]).
...
- Include the attachment files when sending files.
